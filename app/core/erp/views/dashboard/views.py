...
from app.core.erp.models import Sale, Product, DetSale
    ...
    def get_graph_sales_products_year_month(self):
        ...
        try:
            for p in Product.objects.all():
                total = DetSale.objects.filter(
                    sale__date_joined__year=year,
                    sale__date_joined__month=month,
                    prod_id=p.id
                ).aggregate(r=Coalesce(Sum('subtotal'), 0)).get('r')
            ...
            # Manejar la excepción o registrar los detalles
